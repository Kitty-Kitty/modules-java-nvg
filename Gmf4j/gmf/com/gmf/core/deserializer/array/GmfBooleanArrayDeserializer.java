 ...
 * 
 * @Title: GmfBooleanArrayDeserializer.java 
 * @Prject: GmfPraser
 * @Package: com.gmf.core.deserializer.array 
 * @Description: boolean array deserializer 
 * @author: kitty   
 * @date: 2016年1月27日 下午3:15:33 
 * @version: V0.1.0   
 */
package com.gmf.core.deserializer.array;
...
/** 
 * @ClassName: GmfBooleanArrayDeserializer 
 * @Description: boolean array deserializer class
 * @author: kitty
 * @date: 2016年1月27日 下午3:15:33  
 */
public class GmfBooleanArrayDeserializer extends GmfArrayDeserializer {
	@Override
	public boolean deserializer(GmfDeserializerConfigure gdc, Object object, Field f) {
		 ...
		 */
		if (null == value) {
			this.println(gdc, GmfErrorDefined.GMF_ERROR_NOTICE,
					"%s value is null error! ", 
					f.getName());
			return true;
		}
		/**
		...
		if (!clazz.isArray()) {
			this.println(gdc, GmfErrorDefined.GMF_ERROR_NOTICE, 
					"%s isArray() failed! not Array Class.",
					clazz.getName());
			return true;
		}
		...
		if (value instanceof List) {
			...
			if (listSize > 0) {
				...
				for (int i = 0; i < listSize; i++) {
					...
					if (itemObjectValue instanceof Boolean) {
